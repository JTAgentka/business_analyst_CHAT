...
export function useHandleSessionHistory() {
  ...
  // Keep a ref to transcriptItems for use in callbacks
  ...
  function handleHistoryAdded(item: any) {
    ...
    if (itemId && role) {
      ...
      if (guardrailMessage) {
        ...
        // For assistant messages, set a timeout to mark guardrail as passed
        // if it hasn't been marked as failed by then
        if (role === 'assistant') {
          setTimeout(() => {
          ...
          }, 2000); // 2 second delay to allow for guardrail check
        }
      }
    }
  }
  ...
  function handleHistoryUpdated(items: any[]) {
    ...
    items.forEach((item: any) => {
      ...
      const { itemId, content = [], role, status } = item;
      ...
      if (text) {
        ...
        // For assistant messages, check if this is a completion update
        // and update guardrail status if it's still pending
        if (role === 'assistant' && (status === 'completed' || text.length > 0)) {
          const transcriptItem = transcriptItemsRef.current.find((i) => i.itemId === itemId);
          
          // If guardrail is still pending and we have content, mark it as passed
          if (transcriptItem?.guardrailResult?.status === 'IN_PROGRESS') {
            updateTranscriptItem(itemId, {
              status: 'DONE',
              guardrailResult: {
                status: 'DONE',
                category: 'NONE',
                rationale: 'Message passed moderation',
              },
            });
          }
        }
